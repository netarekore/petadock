...
// ── 自動起動登録 ─────────────────────────────────────────────
// NSISインストーラー版: process.execPath が AppData\Local\PetaDock 配下になる
// app.setLoginItemSettings を使用（Electron推奨）
function registerAutoLaunch() {
  if (!app.isPackaged) return;

  const exePath = process.execPath;

  // TempフォルダやWindowsAppsなど不正なパスは登録しない
  const invalidPaths = [
    process.env.TEMP,
    process.env.TMP,
  ...
  if (isInvalidPath) {
    // インストーラー経由でない起動（Tempから実行）は登録しない
    return;
  }
...
function openSettingsWindow() {
  ...
  settingsWin = new BrowserWindow({
    ...
    x: sx, y: sy,
    title: 'PetaDock 設定',
    resizable: false, minimizable: false, alwaysOnTop: false,
    icon: path.join(__dirname, 'icon.ico'),
    webPreferences: {
      nodeIntegration: false, contextIsolation: true,
      preload: path.join(__dirname, 'preload.js'),
...
function registerShortcut(key) {
  ...
  try {
    ...
    if (ok) store.set('shortcutKey', key);
    return ok;
  } catch { return false; }
}
...
app.whenReady().then(() => {
  ...
  // 起動後しばらくしてから更新チェック（ユーザーデータには触れない）
  if (app.isPackaged) {
...
function checkForUpdates() {
  autoUpdater.autoDownload = false; // ユーザーが承認するまでダウンロードしない
  ...
  autoUpdater.on('error', () => {
    // ネットワークエラーなどは静かに無視（ユーザー体験を損なわない）
  });
...
ipcMain.handle('save-history',     (_, history)  => store.set('history', Array.isArray(history) ? history : []));
ipcMain.handle('save-max-history', (_, n)        => store.set('maxHistory', n));
ipcMain.handle('set-shortcut',     (_, key)      => registerShortcut(key));
ipcMain.handle('write-clipboard',  (_, text)     => { clipboard.writeText(text); lastClipboard = text; });
ipcMain.handle('hide-window',      ()            => safeHideWindow(win));
...
ipcMain.handle('check-update',     ()            => { if (app.isPackaged) checkForUpdates(); else dialog.showMessageBox(null, { message: '開発モードでは更新チェックできません。' }); });
...
ipcMain.handle('import-data', async (_, fmt, overwrite) => {
  const data_overwrite = !!overwrite;
  const parent = settingsWin && !settingsWin.isDestroyed() ? settingsWin : win;
  ...
  try {
    if (fmt === 'csv') {
      ...
      for (const line of lines) {
        ...
        cols.push(cur);
        let grp, title, item;
        if (cols.length >= 3) { [grp, title, item] = cols; }
        else { [grp, item] = cols; title = ''; }
        if(!grp||!item) continue;
        if(!groups[grp]) groups[grp]=[];
        if(!groups[grp].find(i=>i.text===item)) groups[grp].push({title:title||'',text:item});
      }
      const tabs = data_overwrite ? [] : (store.get('tabs') || []);
      for (const [name,items] of Object.entries(groups)) {
        const ex = tabs.find(t=>t.name===name);
        if (ex) { for(const i of items) if(!ex.items.find(e=>(e.text||e)===i.text)) ex.items.push(i); }
        else tabs.push({id:'g'+Date.now()+Math.random().toString(36).slice(2,5),name,items,pinned:false});
      }
      store.set('tabs', tabs);
      safeSendToMain('tabs-updated', tabs);
      ...
      if (data.tabs) {
        if (data_overwrite) {
          ...
          for (const tab of data.tabs) {
            ...
            if (ex) { for(const i of tab.items||[]) if(!ex.items.find(e=>(e.text||e)===(i.text||i))) ex.items.push(i); }
            else existing.push(tab);
          }
          store.set('tabs', existing);
        }
        safeSendToMain('tabs-updated', store.get('tabs'));
      }
      if (data.history) store.set('history', Array.isArray(data.history) ? data.history : []);
      return { ok:true, count:(store.get('tabs')||[]).reduce((s,t)=>s+(t.items||[]).length,0) };
    }
...
ipcMain.handle('uninstall', async () => {
  ...
  try { fs.rmSync(app.getPath('userData'), { recursive:true, force:true }); } catch (_) {}
  const exePath = process.execPath;
